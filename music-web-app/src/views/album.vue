<template>
  <div class="album-detail" :style="{'padding-bottom': padBotVal}">
    <music-list
      :songs="songs"
      :title="title"
      :pic="pic"
...
<script>
  ...
  export default {
    ...
    computed: {
      title() {
        return this.albumFromStorage?.title
      },
      pic() {
        return this.albumFromStorage?.pic
      },
    },
    async created () {
      if (this.album) {
        // 如果是通过一级路由传过来的，那么就保存到localstorage中
        const val = storage.get(ALBUM_KEY, {})
        // album.id作为键去查找，这个也就是路由中的一部分
      ...
      this.albumFromStorage = storage.get(ALBUM_KEY, {})[this.$route.params.id]
      if (this.albumFromStorage === undefined) {
        // 如果这个id是用户随便输入的，localstorege中没有那么就返回上一级
        this.$router.push({ path: `${this.$route.matched[0].path}` })
        return
      }
      const result = await getAlbum(this.albumFromStorage)
